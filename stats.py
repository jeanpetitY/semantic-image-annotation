...
def main():
    parser = argparse.ArgumentParser(
        description="Compare labels from a dataset JSON file and an image folder."
    )
    parser.add_argument(
        "json_path",
        nargs="?",
    ...
    parser.add_argument(
        ...
        help=f"Path to the dataset folder. Default: {DEFAULT_DATASET_PATH}",
    )
    parser.add_argument(
        "--label-key",
    ...
    parser.add_argument(
        "--search-usda",
        action="store_true",
        help="Search USDA labels for every not annotated label.",
    )
