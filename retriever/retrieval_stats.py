...
DEFAULT_JSON_PATH = "json/old/uecfood256_usda_enriched.json"
...
def get_retrieval_stats(json_path, dataset_path, label_key=DEFAULT_LABEL_KEY):
    ...
    not_retrieved_labels = [
        label for label in initial_labels if label not in annotated_label_set
    ]

    return annotated_labels, initial_labels, not_retrieved_labels
...
def get_usda_pairs_for_missing_labels(
    json_path,
    dataset_path,
    label_key=DEFAULT_LABEL_KEY,
    sleep_seconds=0.5,
):
    _, _, missing_labels = get_retrieval_stats(
        json_path,
        dataset_path,
        label_key,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare USDA-retrieved labels against an image-dataset label directory."
    )
    parser.add_argument(
        "json_path",
        nargs="?",
        default=DEFAULT_JSON_PATH,
        help=f"Path to the retrieval JSON file. Default: {DEFAULT_JSON_PATH}",
    )
    parser.add_argument(
        "dataset_path",
        nargs="?",
    ...
    parser.add_argument(
        "--search-usda",
        action="store_true",
        help="Search USDA suggestions for labels missing from the retrieval output.",
    )
...
def main():
    ...
    retrieved_labels, initial_labels, missing_labels = get_retrieval_stats(
        args.json_path,
        args.dataset_path,
        args.label_key,
    )

    print(f"Retrieved labels ({len(retrieved_labels)}):")
    ...
    if args.search_usda:
        usda_label_pairs = get_usda_pairs_for_missing_labels(
            args.json_path,
            args.dataset_path,
            args.label_key,
        )
